...
import { getDb, logAudit, named, setSetting } from "@/lib/db";
...
export async function saveVisibility(_prev: ActionState, form: FormData): Promise<ActionState> {
  ...
  for (const switchDef of OPERATOR_SWITCHES) {
    await writeOperatorSwitch(switchDef.key, form.get(switchDef.key) !== null);
  }
  await logAudit("founder", "visibility_changed");
...
export async function saveBusiness(_prev: ActionState, form: FormData): Promise<ActionState> {
  ...
  await setSetting("cash_buffer", buffer);
  await setSetting(
    "business_name",
    String(form.get("business_name") ?? "").trim() || "Neuroid Media",
  );
  await logAudit("founder", "business_settings_changed");
  revalidatePath("/settings");
...
export async function savePnlMonth(_prev: ActionState, form: FormData): Promise<ActionState> {
  ...
  await db.query(
    ...named(
      `INSERT INTO foundery.pnl_months (month, other_income, one_off_costs, tax_rate, notes, closed, updated_at)
       VALUES (@month, @other_income, @one_off_costs, @tax_rate, @notes, @closed, now())
         ...
         updated_at=now()`,
      {
        ...
        closed: Boolean(form.get("closed")),
      },
    ),
  );

  await logAudit("founder", "pnl_month_saved", "month", month);
  revalidatePath("/pnl");
